...
/// <summary>
/// Fetches and caches Identity's public signing keys from its JWKS endpoint (api-contract.yaml's
/// OAuth2 Client Credentials security scheme — this service validates the JWT Identity issued, it
/// never mints or holds a private key itself). Cached for 10 minutes so a validation on every
/// request doesn't cost an HTTP round trip; a fetch failure surfaces as "no matching key," which
/// JwtBearer turns into a 401 rather than a crash (kart-recommendation-service precedent).
/// </summary>
...
{
    private const string CacheKey = "kart-analytics-service:identity-jwks";
